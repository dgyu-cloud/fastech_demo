...
def PrintStatus(nBdID: int) -> bool:
    ...
    while True:
        ...
        status_result, lActPos = FAS_GetActualPos(nBdID)
        if status_result != FMM_OK:
        ...
        print(
            "Position %d : %s" % (
                lActPos,
                "Work Detected!" if dwOutput & SERVO2_OUT_BITMASK_PUSHDETECT else "Work Not Detected"
            )
        )
...
def main():
    ...
    # Print status
    if not PrintStatus(nBdID):
        input("Press Enter to exit...")
        exit(1)

    # Move Stop And Check Motion
    if not StopMotion(nBdID):
        input("Press Enter to exit...")
        exit(1)
