...
def SetDACValue(nBdID: int) -> bool:
    ...
    if FAS_SetDACValue(nBdID, byChannel, bEnable, lDACValue) != FMM_OK:
        ...
        print("Set DAC Value Success")
    return True


def GetDACValue(nBdID: int) -> bool:
    ...
    if status_result != FMM_OK:
        ...
        print("Get DAC Value Success : Enable = %d, DAC = %d" % (Enable, Value))
    return True
...
def main():
    ...
    # Set DAC Value
    if not SetDACValue(nBdID):
        input("Press Enter to exit...")
        exit(1)

    # Get DAC Value
    if not GetDACValue(nBdID):
        input("Press Enter to exit...")
        exit(1)
